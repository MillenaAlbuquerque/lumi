...
import { CalendarDays, ChevronDown, LoaderCircle, Popcorn, Search, X } from 'lucide-react'
...
function MoviesSessionsSection() {
  ...
  useEffect(() => {
    const isRefresh = hasLoadedMovies.current
    setLoading(!isRefresh)
    setFiltering(isRefresh)
    setError('')
    clientShowtimeService.listMovies(selectedDate ? format(selectedDate, 'yyyy-MM-dd') : undefined)
      .then(setMovies)
      .catch((requestError: Error) => setError(requestError.message))
      .finally(() => { hasLoadedMovies.current = true; setLoading(false); setFiltering(false) })
  }, [selectedDate])
  ...
  return <section id="sessoes" className="scroll-mt-28 px-6 py-10 sm:scroll-mt-32">
    ...
    <MovieDetailsModal movie={selectedMovie} showDate={selectedDate ? format(selectedDate, 'yyyy-MM-dd') : undefined} open={!!selectedMovie} onOpenChange={(open) => { if (!open) setSelectedMovie(null) }} onSelectSession={selectSession} />
  </section>
}
